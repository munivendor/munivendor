...
@Component({
  ...
  standalone: true,
  template: `
    <div class="custom-tooltip-dialog"
    [ngStyle]="{ width: width }"
      ...
      <div *ngIf="subtextTwo !== ''" class="tooltip-subtext">{{ subtextTwo }}</div>
      <div class="tooltip-actions">
        <button mat-button (click)="close.emit()">Close</button>
        <button *ngIf="actionLabel" mat-button color="primary" (click)="action.emit()">
          {{ actionLabel }}
        </button>
      </div>
