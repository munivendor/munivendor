...
export class DocumentService {
  ...
  GetAllDocumentInstances(requestId: number): Observable<any> {
    return this.http.get<{ documentInstance: DocumentInstance[] }>(`${this.url}DocumentInstances`, {
      params: new HttpParams().set('requestId', requestId)
    });
  }

  GetAutoFillStatus(requestId: number): Observable<{ isAutoFillComplete: boolean, pollFrequency: number }> {
    return this.http.get<{ isAutoFillComplete: boolean, pollFrequency: number }>(`${this.url}DocumentInstances/AutoFillStatus/${requestId}`);
  }
}